...
class hr_indicadores_previsionales(models.Model):
    ...
    def update_document(self):
        ...
        try:
            ...
            self.sueldo_minimo      = new_ind.get['RENTAS_MINIMA_DEP_INDEP'][0]
            self.sueldo_minimo_otro = new_ind['RENTAS_MINIMA_18_Y_65'][0]

            # Ahorro Previsional Voluntario (UF)
            self.tope_mensual_apv = new_ind.get['APV_TOPE_MENSUAL'][0]
            self.tope_anual_apv   = new_ind.get['APV_TOPE_ANUAL'][0]

            # 5 DEPÓSITO CONVENIDO (UF)
            self.deposito_convenido = new_ind.get['DEPOSITO_CONVENIDO_TOPE_ANUAL'][0]

            # 6 SEGURO DE CESANTÍA (AFC)
            self.contrato_plazo_indefinido_empleador      = new_ind.get['SEGURO_CESANTIA_PLAZO_INDEF'][0]
            self.contrato_plazo_indefinido_trabajador     = new_ind.get['SEGURO_CESANTIA_PLAZO_INDEF'][1]
            self.contrato_plazo_fijo_empleador            = new_ind.get['SEGURO_CESANTIA_PLAZO_FIJO']
            self.contrato_plazo_indefinido_empleador_otro = new_ind.get['SEGURO_CESANTIA_11_ANNOS']

            # 7 ASIGNACIÓN FAMILIAR
            self.asignacion_familiar_monto_a = new_ind.get['ASIGNACION_FAMILIAR_A'][0]
            self.asignacion_familiar_monto_b = new_ind.get['ASIGNACION_FAMILIAR_B'][0]
            self.asignacion_familiar_monto_c = new_ind.get['ASIGNACION_FAMILIAR_C'][0]

            self.asignacion_familiar_primer  = new_ind.get['ASIGNACION_FAMILIAR_A'][1]
            self.asignacion_familiar_segundo = new_ind.get['ASIGNACION_FAMILIAR_B'][1]
            self.asignacion_familiar_tercer  = new_ind.get['ASIGNACION_FAMILIAR_C'][1]

            # 8 TASA COTIZACIÓN OBLIGATORIO AFP
            self.tasa_afp_capital           = new_ind.get['TASA_CAPITAL'][0]
            self.tasa_sis_capital           = new_ind.get['TASA_CAPITAL'][1]
            self.tasa_independiente_capital = new_ind.get['TASA_CAPITAL'][2]

            self.tasa_afp_cuprum           = new_ind.get['TASA_CUPRUM'][0]
            self.tasa_sis_cuprum           = new_ind.get['TASA_CUPRUM'][1]
            self.tasa_independiente_cuprum = new_ind.get['TASA_CUPRUM'][2]

            self.tasa_afp_habitat           = new_ind.get['TASA_HABITAT'][0]
            self.tasa_sis_habitat           = new_ind.get['TASA_HABITAT'][1]
            self.tasa_independiente_habitat = new_ind.get['TASA_HABITAT'][2]

            self.tasa_afp_planvital           = new_ind.get['TASA_PLANVITAL'][0]
            self.tasa_sis_planvital           = new_ind.get['TASA_PLANVITAL'][1]
            self.tasa_independiente_planvital = new_ind.get['TASA_PLANVITAL'][2]

            self.tasa_afp_provida           = new_ind.get['TASA_PROVIDA'][0]
            self.tasa_sis_provida           = new_ind.get['TASA_PROVIDA'][1]
            self.tasa_independiente_provida = new_ind.get['TASA_PROVIDA'][2]

            self.tasa_afp_modelo           = new_ind.get['TASA_MODELO'][0]
            self.tasa_sis_modelo           = new_ind.get['TASA_MODELO'][1]
            self.tasa_independiente_modelo = new_ind.get['TASA_MODELO'][2]

            self.tasa_afp_uno           = new_ind.get['TASA_UNO'][0]
            self.tasa_sis_uno           = new_ind.get['TASA_UNO'][1]
            self.tasa_independiente_uno = new_ind.get['TASA_UNO'][2]
